 ...
 * 
 * This calculator performs pension benefit calculations including:
 * - User input processing
 * - MGA (Maximum des gains admissibles) data handling
...
class UserInput {
    constructor() {
        // Example data, will be overridden by user input
        this.salary = [
...
function calculatePensionBenefits(F1, D2, userObj, mgaObj, Protection = false, lastYearOnly = false) {
    ...
    const out_V1 = new Array(55).fill(0);
    const out_V2 = new Array(55).fill(0);
    ...
    for (let i = 0; i < 55; i++) {
        ...
        if (annee[i] >= 2019) {
            sal[i] = (rajustes_2[i] + (annee[i]/100000)) * v1[i];
        } else {
            sal[i] = (annee[i] / 100000) * v1[i];
        }
    }
    ...
    // PHASE 11b: Calculate out_V1
    for (let i = 0; i < 55; i++) {
        if ((rg[i] <= 40) && (mois[i] < 12)) {
            out_V1[i] = mois[i];
        } else {
            out_V1[i] = 0;
        }
    ...
    for (let i = 0; i < 55; i++) {
        ...
        if (rg[i] <= 40) {
            x = 1;
        } else if ((rg[i] === 41) && (Math.max(...out_V1) > 0)) {
            x = (12 - (Math.max(...out_V1))) / 12;
        } else {
            x = 0;
        }
    ...
    // PHASE 13b: Calculate out_V2
    for (let i = 0; i < 55; i++) {
        if ((col_aa[i] <= 40) && (mois[i] < 12)) {
            out_V2[i] = mois[i];
        } else {
            out_V2[i] = 0;
        }
    ...
    for (let i = 0; i < 55; i++) {
        if (col_aa[i] <= 40) {
            volet_2[i] = col_z[i];
        } else if ((col_aa[i] === 41) && (Math.max(...out_V2) > 0)) {
            volet_2[i] = col_z[i] * (12 - Math.max(...out_V2)) / 12;
        } else {
            volet_2[i] = 0;
        }
    }
